# Harness logos (landing)

Vendored SVG marks for the “works with” row. Sources:

| File | Source |
| --- | --- |
| `cursor.svg` | [Simple Icons](https://github.com/simple-icons/simple-icons) (CC0) |
| `grok.svg` | xAI monochrome mark (same 24×24 simple-mark style as Simple Icons; Grok Bot is not a Simple Icons slug yet) |
| `looper-bot.png` | Grok Bot operator hero shot on `/harnesses/#grok-bot` (426×426 screenshot; rounded display, not cube-cropped) |
| `grok-bot-hex.png` | xAI Grok Bot share-card hex avatar (flat green hex with eyes; source SVG: `grok-bot-hex.svg`; kept for other uses) |
| `cline.svg` | Simple Icons (CC0) |
| `openai.svg` | Simple Icons (CC0) — Codex / OpenAI |
| `deepseek.svg` | Simple Icons (CC0) — DSH |
| `opencode.svg` | [OpenCode brand](https://opencode.ai/brand) — `anomalyco/opencode` `opencode-logo-light-square.svg` |
| `pi.svg` | [Pi website](https://github.com/earendil-works/pi-website) `src/logo.svg` |
| `meta.svg` | [Simple Icons](https://github.com/simple-icons/simple-icons) (CC0) — Muse Code / Meta |
| `claude.svg` | [Simple Icons](https://github.com/simple-icons/simple-icons) `claude` slug (CC0) — Claude starburst symbol (not Anthropic corporate A) |

Simple Icons and OpenCode mark recolored with `currentColor` for the dark landing page.

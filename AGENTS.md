...
- Cursor-only dogfood: worker `composer-2.5`, judge `grok-4.6` via `reviewModel` (default `reviewRuntime: cursor`).
- Optional peers: `@cursor/sdk` required for cursor runtime; `@cline/sdk` only for Cline paths; `@opencode-ai/sdk` + `opencode-ai` CLI for OpenCode; `@earendil-works/pi-coding-agent` for `runtime: pi`; `@openai/codex-sdk` for `runtime: codex`; PATH `dsh` for `runtime: dsh` (no `@deepseek-ai/dsh` on the CLI package). Primary judge can use any of those via `reviewRuntime`.

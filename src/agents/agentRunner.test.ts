...
describe('createLoopAgentSession', () => {
  beforeEach(() => {
    vi.resetAllMocks()
    runCursorAgentPrompt.mockResolvedValue('cursor-ok')
    createClineLoopSession.mockResolvedValue({
      runPrompt: vi.fn().mockResolvedValue('cline-ok'),
  ...
  it('dispatches cursor runtime to runCursorAgentPrompt', async () => {
    ...
    const text = await session.runIterationPrompt(
      'prompt',
      { runtime: 'cursor', model: 'composer-2.5' },
      { assistantOutput: 'none' },
    )

    expect(text).toBe('cursor-ok')

...
The same shared skills (via symlinks) plus DSH-only `run-loop-in-dsh` and a `loop-scaffold` command ship as a DSH Cordis bundle at [`plugins/dsh-agent-looper/`](../plugins/dsh-agent-looper/) — see [`docs/dsh-plugin.md`](./dsh-plugin.md). Shared skill bodies live only under `plugins/agent-looper/skills/`.

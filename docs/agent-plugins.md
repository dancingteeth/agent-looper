...
DeepSeek Harness companion (skills + human command, Cordis bundle): [`plugins/dsh-agent-looper/`](../plugins/dsh-agent-looper/) — see [`docs/dsh-plugin.md`](./dsh-plugin.md). Shared skill bodies are copied from [`plugins/agent-looper/skills/`](../plugins/agent-looper/skills/) (SSOT) by `materialize-skills`; only `run-loop-in-dsh` is DSH-native.

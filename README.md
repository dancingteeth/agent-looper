...
# or: pnpm exec agent-loop-prompt --out .cursor/loops/my-task
# edit GOAL.md
...
How the loop is shaped: [`README.intro.md`](./README.intro.md). Technical deep dive: [`ARCHITECTURE.md`](./ARCHITECTURE.md) (including §1.1 — the harness is a small control-flow graph; the Ralph loop lives inside the worker node). npm releases: [`docs/releasing.md`](./docs/releasing.md). What shipped: [`CHANGELOG.md`](./CHANGELOG.md) (current **0.7.0**). Embedding this package as a library dependency in another ADE: [`docs/embed-api.md`](./docs/embed-api.md). Reporting a vulnerability: [`SECURITY.md`](./SECURITY.md).
...
# Claude Code (needs `claude` 2.1.169+ on PATH; `claude login`)
# leave ANTHROPIC_API_KEY unset so -p uses subscription quota
pnpm exec agent-check claude
pnpm exec agent-loop run .cursor/loops/my-task --runtime claude
# typical: cheap worker + Claude judge — docs/claude-runtime.md
```
...
| Field | Default | Purpose |
| --- | --- | --- |
| `runtime` | `cursor` | Worker: `cursor` \| `cline-pass` \| `cline` \| `opencode` \| `pi` \| `codex` \| `dsh` \| `muse` \| `claude`. Unset when `costPreset` is set so detection can bind. See [`docs/runtime-map.md`](./docs/runtime-map.md). Same-task cost method: [`docs/runtime-cost-bench.md`](./docs/runtime-cost-bench.md). |
...
| `model` / `escalateModel` | (defaults) | Worker model; escalate on identical verifier stagnation **or** immediately after a hung/timed-out worker (OpenCode/Pi/Codex/DSH/Muse/Claude: after threshold; Cline: after reasoning ceiling — worker fault skips the ceiling). Cursor workers are always Composer 2.5 and have no escalate slug — omit `escalateModel` (repo defaults that leak a Go escalate are dropped). The judge is `reviewRuntime` / `reviewModel`, not `escalateModel`. |
...
| `reasoningEffort` | — | `low` \| `medium` \| `high` \| `xhigh` \| `none` when the runtime honors it (Cline, Pi, Muse). Omit or `none` = no extra thinking. Cursor / OpenCode / Codex / DSH / Claude ignore it. |
...
| `reviewModel` | (resolved) | Judge model for `reviewRuntime`. Cursor defaults: `grok-4.7` when worker is `cursor`, else `composer-2.5` (Grok 4.6 and 4.5 still allowed). OpenCode judge (`reviewRuntime: "opencode"`) defaults to **`opencode-go/deepseek-v4-pro`** (worker stays Flash). DSH judge (`reviewRuntime: "dsh"`) defaults to **`deepseek-official/deepseek-v4-pro`**. Codex judge (`reviewRuntime: "codex"`) defaults to **`gpt-5.6-sol`** (worker stays Luna; optional `gpt-6-astra`). Muse judge (`reviewRuntime: "muse"`) defaults to **`muse-spark-1.3`** (worker stays contributor). Claude judge (`reviewRuntime: "claude"`) defaults to **`opus`** (Claude Code’s latest Opus, currently Opus 5; worker stays Sonnet). Pi / Cline judges use that runtime’s worker default. Never Composer Fast on cursor. |
...
| `agent-check cursor\|cline\|opencode\|pi\|codex\|dsh\|muse\|claude` | SDK + API key smoke (`dsh`: PATH CLI + Node ≥ 22.15; `muse`: PATH `muse` + `@muse-code/sdk`; `claude`: PATH CLI 2.1.169+ + `--safe-mode`, no quota spend) |

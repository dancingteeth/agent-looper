export function loopScaffoldGuidance(agentLoopBinary: string, rawInput: string): string {
  ...
  return [
    ...
    '3. Copy harness templates (`GOAL.template.md`, `verify.example.sh`, `loop.json.example`) or run `pnpm exec agent-loop-init` if the repo has no profile yet. If a human will *look* at the result (homepage, mockup, screenshot-as-hero), use `GOAL.visual.template.md` — do not copy `postQualityReview` / `reviewGate` from `*-smoke` or `example-fix`.',
